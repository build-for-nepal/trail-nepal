...
const HikeHeader = ({ hike }: { hike: HikeDetail }) => {
  const bgImage =
    hike.gallery?.find((img) => img.type === 'hero')?.url ||
    hike.gallery?.[0]?.url ||
    '';

  const stats = [
    { icon: Clock, label: 'Duration', value: hike.meta.duration },
    { icon: TrendingUp, label: 'Difficulty', value: hike.meta.difficulty },
    { icon: Mountain, label: 'Max Elevation', value: hike.meta.maxElevation },
    {
      icon: CalendarDays,
      label: 'Peak Season',
      value: formatBestSeason(hike.meta.bestSeasons),
    },
    { icon: MapPin, label: 'Starting Point', value: hike.meta.startingPoint },
  ];
